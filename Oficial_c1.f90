...
do ni = 1, 1
    ...
    open(unit = 406, file = 'em9'//nfac//'ANI.dat')
    open(unit = 407, file = 'EvSL.dat')  ! um teste
...
real function v8(x)
    real*8, intent(in) :: x(:)
    v8 = cAni * vac(x) * kox(4.5d-1, 3d-1, x(5))
end function

real function v8r(x)
    real*8, intent(in) :: x(:)
    v8r = x(5) * kred(4.5d-1, 6d-1, x(5))
end function
